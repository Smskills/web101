...
const FAQPage: React.FC<FAQPageProps> = ({ faqsState, contact }) => {
  ...
  // Normalize data: Handle cases where faqsState might be null, an array (legacy), or the new object shape
  const faqData = faqsState || { list: [], pageMeta: { title: 'Help Center', subtitle: '', tagline: 'FAQ' } };
  const list = Array.isArray(faqData.list) ? faqData.list : (Array.isArray(faqData) ? faqData as unknown as FAQItem[] : []);
  const pageMeta = faqData.pageMeta || { title: 'Help Center', subtitle: '', tagline: 'Institutional FAQ' };
  ...
  // Safe filtering: Ensure strings exist before calling toLowerCase()
      ...
      <section className="bg-slate-900 pt-32 pb-24 text-white relative overflow-hidden text-center">
        ...
        <div className="container mx-auto px-4 relative z-10 max-w-4xl">
          <span className="text-emerald-500 font-black uppercase tracking-[0.4em] text-[10px] mb-4 block">
            {pageMeta.tagline || 'Institutional Assistance'}
          </span>
          <h1 className="text-5xl md:text-7xl font-black mb-8 tracking-tighter leading-none">
            {pageMeta.title || 'Help Center'}
          </h1>
          <p className="text-slate-400 text-xl font-medium max-w-2xl mx-auto">
            {pageMeta.subtitle || 'Find answers to common questions about enrollment, curriculum, and placement services.'}
          </p>
