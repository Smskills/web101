    ...
    <div className="min-h-screen bg-slate-900 text-slate-100 pb-20">
      ...
      <div className="container mx-auto px-4 mt-8 flex flex-col md:flex-row gap-8">
        ...
        <div className="flex-grow bg-slate-800 rounded-[2.5rem] p-8 md:p-12 border border-slate-700 shadow-3xl min-h-[70vh]">
          ...
          {activeTab === 'gallery' && <GalleryTab gallery={localContent.gallery} galleryMetadata={localContent.galleryMetadata} updateGalleryItem={(id, f, v) => setLocalContent(prev => ({ ...prev, gallery: prev.gallery.map(i => i.id === id ? { ...i, [f]: v } : i) }))} deleteItem={(id) => { if(window.confirm('Delete this image?')) deleteItem('gallery', id); }} triggerUpload={(cat) => { activeUploadCategory.current = cat; galleryInputRef.current?.click(); }} triggerThumbnailUpload={(cat) => { activeThumbnailCategory.current = cat; thumbnailInputRef.current?.click(); }} />}
          {activeTab === 'faq' && <FAQTab faqs={localContent.faqs} updateFAQ={(id, f, v) => setLocalContent(prev => ({ ...prev, faqs: prev.faqs.map(item => item.id === id ? { ...item, [f]: v } : item) }))} addFAQ={() => setLocalContent(prev => ({ ...prev, faqs: [{ id: Date.now().toString(), question: 'New Question?', answer: 'Answer...', category: 'General' }, ...prev.faqs] }))} deleteFAQ={(id) => { if(window.confirm('Delete this FAQ?')) deleteItem('faqs', id); }} reorderFAQs={(s, e) => { if (e < 0 || e >= localContent.faqs.length) return; setLocalContent(prev => { const res = Array.from(prev.faqs); const [rem] = res.splice(s, 1); res.splice(e, 0, rem); return { ...prev, faqs: res }; }); }} />}
          ...
          {activeTab === 'placements' && <PlacementsTab stats={localContent.placements.stats} reviews={localContent.placements.reviews} partners={localContent.placements.partners} pageDescription={localContent.placements.pageDescription} updateStat={(id, f, v) => setLocalContent(prev => ({ ...prev, placements: { ...prev.placements, stats: prev.placements.stats.map(s => s.id === id ? { ...s, [f]: v } : s) } }))} addStat={() => setLocalContent(prev => ({ ...prev, placements: { ...prev.placements, stats: [...prev.placements.stats, { id: Date.now().toString(), label: 'Stat', value: '0', icon: 'fa-chart-simple' }] } }))} deleteStat={(id) => { if(window.confirm('Delete this statistic?')) setLocalContent(prev => ({ ...prev, placements: { ...prev.placements, stats: prev.placements.stats.filter(s => s.id !== id) } })); }} updateReview={(id, f, v) => setLocalContent(prev => ({ ...prev, placements: { ...prev.placements, reviews: prev.placements.reviews.map(r => r.id === id ? { ...r, [f]: v } : r) } }))} addReview={() => setLocalContent(prev => ({ ...prev, placements: { ...prev.placements, reviews: [{ id: Date.now().toString(), name: 'Student', course: 'Track', company: 'Company', companyIcon: 'fa-building', image: 'https://i.pravatar.cc/150', text: '', salaryIncrease: '' }, ...prev.placements.reviews] } }))} deleteReview={(id) => { if(window.confirm('Delete this review?')) setLocalContent(prev => ({ ...prev, placements: { ...prev.placements, reviews: prev.placements.reviews.filter(r => r.id !== id) } })); }} updatePartner={(id, f, v) => setLocalContent(prev => ({ ...prev, placements: { ...prev.placements, partners: prev.placements.partners.map(p => p.id === id ? { ...p, [f]: v } : p) } }))} addPartner={() => setLocalContent(prev => ({ ...prev, placements: { ...prev.placements, partners: [...prev.placements.partners, { id: Date.now().toString(), name: 'New Partner', icon: 'fa-building' }] } }))} deletePartner={(id) => { if(window.confirm('Delete this partner?')) setLocalContent(prev => ({ ...prev, placements: { ...prev.placements, partners: prev.placements.partners.filter(p => p.id !== id) } })); }} updatePageDescription={(v) => updateField('placements', 'pageDescription', v)} onReviewImageClick={(id) => { activeReviewId.current = id; reviewInputRef.current?.click(); }} onPartnerImageClick={(id) => { activePartnerId.current = id; partnerInputRef.current?.click(); }} label={localContent.home.sectionLabels.placementMainLabel} />}
