...
const AdminDashboard: React.FC<AdminDashboardProps> = ({ content, onUpdate }) => {
  ...
  // List Management - Fixed to handle nested list object
  const addItem = (section: 'courses' | 'notices' | 'gallery', item: any) => {
    setLocalContent(prev => ({ 
      ...prev, 
      [section]: { 
        ...(prev[section] as any), 
        list: [{ ...item, id: Date.now().toString() }, ...((prev[section] as any).list)] 
      } 
    }));
  };
  ...
  const deleteItem = (section: 'courses' | 'notices' | 'gallery' | 'faqs', id: string) => {
    setLocalContent(prev => ({ 
      ...
      [section]: { 
        ...(prev[section] as any), 
        list: ((prev[section] as any).list).filter((item: any) => item.id !== id) 
      } 
    }));
  };
  ...
  return (
    <div className="min-h-screen bg-slate-900 text-slate-100 pb-20">
      {/* Hidden Upload Inputs - Fixed nested list access for uploads */}
      ...
      <input type="file" ref={courseInputRef} className="hidden" accept="image/*" onChange={(e) => handleUpload(e, (url) => activeCourseId.current && setLocalContent(prev => ({ ...prev, courses: { ...prev.courses, list: prev.courses.list.map(c => c.id === activeCourseId.current ? { ...c, image: url } : c) } })))} />
      <input type="file" ref={galleryInputRef} className="hidden" accept="image/*" onChange={(e) => handleUpload(e, (url) => setLocalContent(prev => ({ ...prev, gallery: { ...prev.gallery, list: [{ id: Date.now().toString(), url, category: activeUploadCategory.current, title: '' }, ...prev.gallery.list] } })))} />
      ...
      <div className="container mx-auto px-4 mt-8 flex flex-col md:flex-row gap-8">
        ...
        <div className="flex-grow bg-slate-800 rounded-[2.5rem] p-8 md:p-12 border border-slate-700 shadow-3xl min-h-[70vh]">
          ...
          {activeTab === 'courses' && <CoursesTab courses={localContent.courses.list} updateCourseItem={(id, f, v) => setLocalContent(prev => ({ ...prev, courses: { ...prev.courses, list: prev.courses.list.map(c => c.id === id ? { ...c, [f]: v } : c) } }))} onCourseImageClick={(id) => { activeCourseId.current = id; courseInputRef.current?.click(); }} addItem={() => addItem('courses', { name: 'New Track', duration: '6 Months', mode: 'Hybrid', description: 'Program details...', status: 'Active', image: 'https://picsum.photos/800/600', price: '$1,000' })} deleteItem={(id) => { if(window.confirm('Delete this course?')) deleteItem('courses', id); }} />}
          {activeTab === 'notices' && <NoticesTab notices={localContent.notices.list} updateNoticeItem={(id, f, v) => setLocalContent(prev => ({ ...prev, notices: { ...prev.notices, list: prev.notices.list.map(n => n.id === id ? { ...n, [f]: v } : n) } }))} addItem={() => addItem('notices', { date: new Date().toISOString().split('T')[0], title: 'New Notice', description: 'Official text...', isImportant: false, category: 'General' })} deleteItem={(id) => { if(window.confirm('Delete this notice?')) deleteItem('notices', id); }} />}
          {activeTab === 'gallery' && <GalleryTab gallery={localContent.gallery.list} galleryMetadata={localContent.galleryMetadata} updateGalleryItem={(id, f, v) => setLocalContent(prev => ({ ...prev, gallery: { ...prev.gallery, list: prev.gallery.list.map(i => i.id === id ? { ...i, [f]: v } : i) } }))} deleteItem={(id) => { if(window.confirm('Delete this image?')) deleteItem('gallery', id); }} triggerUpload={(cat) => { activeUploadCategory.current = cat; galleryInputRef.current?.click(); }} triggerThumbnailUpload={(cat) => { activeThumbnailCategory.current = cat; thumbnailInputRef.current?.click(); }} />}
          {activeTab === 'faq' && <FAQTab faqs={localContent.faqs.list} updateFAQ={(id, f, v) => setLocalContent(prev => ({ ...prev, faqs: { ...prev.faqs, list: prev.faqs.list.map(item => item.id === id ? { ...item, [f]: v } : item) } }))} addFAQ={() => setLocalContent(prev => ({ ...prev, faqs: { ...prev.faqs, list: [{ id: Date.now().toString(), question: 'New Question?', answer: 'Answer...', category: 'General' }, ...prev.faqs.list] } }))} deleteFAQ={(id) => { if(window.confirm('Delete this FAQ?')) deleteItem('faqs', id); }} reorderFAQs={(s, e) => { if (e < 0 || e >= localContent.faqs.list.length) return; setLocalContent(prev => { const res = Array.from(prev.faqs.list); const [rem] = res.splice(s, 1); res.splice(e, 0, rem); return { ...prev, faqs: { ...prev.faqs, list: res } }; }); }} />}
          {activeTab === 'form' && <FormTab formData={localContent.enrollmentForm} addField={() => setLocalContent(prev => ({ ...prev, enrollmentForm: { ...prev.enrollmentForm, fields: [...(prev.enrollmentForm?.fields || []), { id: Date.now().toString(), label: 'New Field', type: 'text', placeholder: '', required: false }] } }))} updateField={(id, up) => setLocalContent(prev => ({ ...prev, enrollmentForm: { ...prev.enrollmentForm, fields: (prev.enrollmentForm?.fields || []).map(f => f.id === id ? { ...f, ...up } : f) } }))} deleteField={(id) => { if(window.confirm('Remove this application field?')) setLocalContent(prev => ({ ...prev, enrollmentForm: { ...prev.enrollmentForm, fields: (prev.enrollmentForm?.fields || []).filter(f => f.id !== id) } })); }} updatePageInfo={(f, v) => setLocalContent(prev => ({ ...prev, enrollmentForm: { ...prev.enrollmentForm, [f]: v } }))} />}
          {activeTab === 'contact' && <ContactTab contact={localContent.site.contact} social={localContent.site.social} updateContactField={updateContactField} addSocialLink={() => setLocalContent(prev => ({ ...prev, site: { ...prev.site, social: [...prev.site.social, { id: Date.now().toString(), platform: 'Platform', url: '#', icon: 'fa-globe' }] } }))} updateSocialLink={(id, f, v) => setLocalContent(prev => ({ ...prev, site: { ...prev.site, social: prev.site.social.map(s => s.id === id ? { ...s, [f]: v } : s) } }))} removeSocialLink={(id) => { if(window.confirm('Remove this social link?')) setLocalContent(prev => ({ ...prev, site: { ...prev.site, social: prev.site.social.filter(s => s.id !== id) } })); }} />}
          {activeTab === 'footer' && <FooterTab footer={localContent.site.footer} updateFooterField={(f, v) => setLocalContent(prev => ({ ...prev, site: { ...prev.site, footer: { ...prev.site.footer, [f]: v } } }))} addSupportLink={() => setLocalContent(prev => ({ ...prev, site: { ...prev.site, footer: { ...prev.site.footer, supportLinks: [...prev.site.footer.supportLinks, { label: 'New Link', path: '#' }] } } }))} updateSupportLink={(idx, f, v) => setLocalContent(prev => ({ ...prev, site: { ...prev.site, footer: { ...prev.site.footer, supportLinks: prev.site.footer.supportLinks.map((l, i) => i === idx ? { ...l, [f]: v } : l) } } }))} deleteSupportLink={(idx) => { if(window.confirm('Remove this support link?')) setLocalContent(prev => ({ ...prev, site: { ...prev.site, footer: { ...prev.site.footer, supportLinks: prev.site.footer.supportLinks.filter((_, i) => i !== idx) } } })); }} />}
          ...
          {activeTab === 'legal' && <LegalTab legal={localContent.legal} updateLegal={(p, f, v) => setLocalContent(prev => ({ ...prev, legal: { ...prev.legal, [p]: { ...prev.legal[p], [f]: v } } }))} updateSection={(p, id, f, v) => setLocalContent(prev => ({ ...prev, legal: { ...prev.legal, [p]: { ...prev.legal[p], sections: prev.legal[p].sections.map(s => s.id === id ? { ...s, [f]: v } : s) } } }))} addSection={(p) => setLocalContent(prev => ({ ...prev, legal: { ...prev.legal, [p]: { ...prev.legal[p], sections: [...prev.legal[p].sections, { id: Date.now().toString(), title: 'New', content: '' }] } } }))} deleteSection={(p, id) => { if(window.confirm('Delete this legal section?')) setLocalContent(prev => ({ ...prev, legal: { ...prev.legal, [p]: { ...prev.legal[p], sections: prev.legal[p].sections.filter(s => s.id !== id) } } })); }} />}
          {activeTab === 'career' && <CareerTab career={localContent.career} updateHero={(f, v) => setLocalContent(prev => ({ ...prev, career: { ...prev.career, hero: { ...prev.career.hero, [f]: v } } }))} updateCta={(f, v) => setLocalContent(prev => ({ ...prev, career: { ...prev.career, cta: { ...prev.career.cta, [f]: v } } }))} updateService={(id, f, v) => setLocalContent(prev => ({ ...prev, career: { ...prev.career, services: prev.career.services.map(s => s.id === id ? { ...s, [f]: v } : s) } }))} addService={() => setLocalContent(prev => ({ ...prev, career: { ...prev.career, services: [...prev.career.services, { id: Date.now().toString(), title: 'New Service', description: '', icon: 'fa-compass' }] } }))} deleteService={(id) => { if(window.confirm('Delete this career service?')) setLocalContent(prev => ({ ...prev, career: { ...prev.career, services: prev.career.services.filter(s => s.id !== id) } })); }} onHeroBgClick={() => careerHeroBgInputRef.current?.click()} onServiceImageClick={(id) => { activeCareerServiceId.current = id; careerServiceInputRef.current?.click(); }} />}
        </div>
      </div>
    </div>
  );
